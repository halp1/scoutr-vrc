...
const renderInlineText = (
	...
	return parts.map((part, i) => {
		...
		if (m) {
			return (
				<Text key={`${keyPrefix}-r${i}`} style={styles.ruleRef} onPress={() => onRuleRef(m[1])}>
					{part}
				</Text>
			);
		}
			...
			{nodes.map((node, idx) => {
				...
				if (tag === 'p') {
					const imgInP = node.childNodes.find(
						(c): c is HTMLElement => c instanceof HTMLElement && c.tagName.toLowerCase() === 'img'
					);
					if (imgInP) {
						const src = imgInP.getAttribute('src');
						if (src) {
							const resolved = resolveImageUri(src.startsWith('//') ? `https:${src}` : src);
							return (
...
export const QnAQuestionDetail = ({ question, visible, onClose }: Props) => {
	...
	return (
		<>
					...
					<View style={styles.sheet}>
						...
						<ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
							...
							<Text style={styles.meta}>
								{question.author || 'Unknown author'} · {question.season || 'Unknown season'} ·
								Asked {formatDate(question.askedTimestampMs)}
							</Text>
			...
			{viewerSrc ? (
				<ImageViewerModal src={viewerSrc} alt="Q&A image" onClose={() => setViewerSrc(null)} />
			) : null}
			<RuleDrawer
				entry={ruleEntry}
				visible={ruleVisible}

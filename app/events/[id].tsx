...
const formatMatchName = (match: MatchObj): string => {
  ...
  if (raw.length > 0) {
    ...
    const r16 = parseElim(
      /^(?:round\s*of\s*16|r16)\s*#?\s*(\d+)(?:\s*-\s*(\d+))?$/i,
      "R16",
    );
    if (r16) return r16;
    const qf = parseElim(
...
export default function EventScreen() {
  ...
  const [matchDrawerReturnRow, setMatchDrawerReturnRow] = useState<ScheduleRow | null>(
    null,
  );

  const [predictionsEnabled, setPredictionsEnabled] = useState(false);
  const [vdaMap, setVdaMap] = useState<Map<string, VDAStats> | null>(null);
  ...
  const divisionRatings = useMemo(() => {
    ...
    return calculateOprDprCcwm(
      robotEventsMatchesToScoredMatches(quals, { includeUnscored: true }),
    );
  }, [rawMatches]);
  ...
  const selectedTeamSkills = useMemo(
    () =>
      selectedTeamKey
        ? (skillsRows.find((r) => r.team.trim().toLowerCase() === selectedTeamKey) ??
          null)
        : null,
  ...
  useEffect(() => {
    ...
    re.events
      ...
      .catch((e) => {
        if (!cancelled)
          setLoadError((e as Error).message || "Unable to load event data.");
      })
      .finally(() => {
  ...
  const togglePredictions = async () => {
    const next = !predictionsEnabled;
    setPredictionsEnabled(next);
    if (next && vdaMap === null && !vdaLoading && eventMeta) {
      setVdaLoading(true);
      try {
        const seasons = await re.depaginate(
          re.season.seasonGetSeasons(
            { program: [eventMeta.program.id!] },
            re.custom.maxPages,
          ),
        ...
        // silently fail — predictions just won't show
      } finally {
        setVdaLoading(false);
      }
    }
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.topBar}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          ...
          <TouchableOpacity style={styles.searchBtn} onPress={togglePredictions}>
            {vdaLoading ? (
              <ActivityIndicator size="small" color={colors.primary} />
            ) : (
              <Zap
                size={18}
                color={predictionsEnabled ? colors.primary : colors.mutedForeground}
                fill={predictionsEnabled ? colors.primary : "transparent"}
              />
            )}
          </TouchableOpacity>

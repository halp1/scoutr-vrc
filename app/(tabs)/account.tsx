...
import { requestAndStoreToken } from "../../lib/hooks/useNotificationSetup";
import {
...
export default function AccountScreen() {
  const {
    ...
    removeScoutingTeam,
    notifications,
    setNotificationMyTeam,
  } = useStorage();

  // ── VRC team config ─────────────────────────────────────────────────
  const [teamInput, setTeamInput] = useState(team ?? "");
  const [foundTeam, setFoundTeam] = useState<{ number: string; teamName: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        const match =
          res.find((r) => r.number.toLowerCase() === teamInput.toLowerCase()) ??
          res[0] ??
          null;
        setFoundTeam(
          match
            ? { number: match.number, teamName: (match.teamName ?? "").trim() }
            : null,
        );
      } catch {
        setFoundTeam(null);
  ...
  const handleSaveName = async () => {
    ...
    setAuth({
      ...
      user: {
        ...auth!.user,
        user_metadata: { ...auth!.user.user_metadata, full_name: trimmed },
      },
    });
  ...
  const handleToggleMyTeamNotif = async (value: boolean) => {
    if (value) {
      const granted = await requestAndStoreToken();
      if (!granted) {
        Alert.alert(
          "Permission denied",
      ...
      if (team && program) {
        try {
          ...
          const found =
            results.find((t) => t.number.toLowerCase() === team.toLowerCase()) ??
            results[0];
          if (found) {
            await addNotificationPreference(found.id, found.number, found.program.id);
          }
        } catch {}
      }
    } else {
      if (team && program) {
        try {
          ...
          const found =
            results.find((t) => t.number.toLowerCase() === team.toLowerCase()) ??
            results[0];
          if (found) await removeNotificationPreference(found.id);
        } catch {}
      }
    }
    setNotificationMyTeam(value);
  };
  ...
  const handleLeave = (teamId: string, teamName: string) => {
    Alert.alert(
      "Leave team",
      `Leave "${teamName}"? You can rejoin with the invite code.`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Leave",
            ...
            {!searchingTeam && foundTeam && (
              <View style={styles.foundTeamRow}>
                ...
                <TouchableOpacity
                  style={[
                    styles.actionBtn,
                    { marginTop: 0, paddingHorizontal: spacing.md },
                  ]}
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      ...
      >
        ...
        <View style={styles.card}>
          ...
          {nameError ? (
            <Text style={[styles.errorText, { marginTop: 6 }]}>{nameError}</Text>
          ) : null}
        </View>
        ...
        {team && (
          <>
            ...
            <View style={styles.card}>
              ...
              <View style={[styles.foundTeamRow, { marginTop: spacing.sm }]}>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.foundTeamNumber, { fontSize: font.sm }]}>
                    My Team ({team})
                  </Text>
                  <Text style={[styles.foundTeamName, { fontSize: font.xs }]}>
                    Match results and awards
                  </Text>
                </View>
                <Switch
                  value={notifications.myTeam}
                  onValueChange={handleToggleMyTeamNotif}

...
export default function ExploreScreen() {
  ...
  const [loadPageFn, setLoadPageFn] = useState<
    ((page: number) => Promise<SearchEvent[]>) | null
  >(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const latestSeason = pool.reduce((a, b) => (a.start! > b.start! ? a : b));
  ...
  const doSearch = useCallback(
    (
      ...
      (async () => {
        ...
        try {
          const [teamRes, eventRes] = await Promise.allSettled([
            teamsPromise,
            eventsPromise,
          ]);
  ...
  const navigateToEvent = async (sku: string) => {
    try {
      const results = await re.depaginate(
        re.events.eventGetEvents({ sku: [sku] }, re.custom.maxPages),
      ...
      <View style={styles.header}>
        ...
        <View style={styles.searchRow}>
          ...
          >
            <SlidersHorizontal
              size={18}
              color={region ? colors.primary : colors.foreground}
            />
          </TouchableOpacity>
        </View>
                    ...
                    >
                      <Text
                        style={[
                          ...
                          active && styles.regionItemTextActive,
                        ]}
                      >
                        {r.name}
                      </Text>

  ...
  useEffect(() => {
    re.depaginate(
      ...
      const filtered = all.filter((p) =>
        CONSTANTS.SUPPORTED_PROGRAMS.includes(p.abbr ?? ""),
      );
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        ...
        const match =
          res.find((t) => t.number.toLowerCase() === teamNumber.toLowerCase()) ??
          res[0] ??
          null;
        setFoundTeam(match);
      } catch {
            ...
            {programs.map((p) => (
              ...
              >
                <Text
                  style={[
                    styles.pillText,
                    selectedProgram?.id === p.id && styles.pillTextActive,
                  ]}
                >
                  {p.abbr}
                </Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text
        style={[
          styles.subtitle,
          { textAlign: "center", marginHorizontal: 32, marginBottom: 16 },
        ]}
      >

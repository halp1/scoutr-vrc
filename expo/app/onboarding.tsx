...
import type { Program, Team } from '../lib/robotevents';
...
export default function OnboardingScreen() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        const res = await re.teams.search({ number: [teamNumber], program: [selectedProgram.id] });
        const match = res.find((t) => t.number.toLowerCase() === teamNumber.toLowerCase()) ?? res[0] ?? null;
        setFoundTeam(match);
      } catch {
        setFoundTeam(null);
      } finally {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ width: '100%', alignItems: 'center' }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.programRow} contentContainerStyle={{ gap: 8 }}>
            {programs.map((p) => (
              <TouchableOpacity
                key={p.id}
              ...
              >
                <Text style={[styles.pillText, selectedProgram?.id === p.id && styles.pillTextActive]}>
                  {p.abbr}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <TouchableOpacity style={styles.skipBtn} onPress={() => { setTeam(null); setOnboarding('team', true); }}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text style={[styles.subtitle, { textAlign: 'center', marginHorizontal: 32, marginBottom: 16 }]}>
      ...
      <TouchableOpacity
        ...
        onPress={() => { setAuth(null); setOnboarding('account', true); }}
      >
        <Text style={styles.skipText}>Skip this step</Text>
      </TouchableOpacity>
    </SafeAreaView>

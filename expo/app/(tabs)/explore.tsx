...
export default function ExploreScreen() {
  ...
  const [loadPageFn, setLoadPageFn] = useState<((page: number) => Promise<SearchEvent[]>) | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const prog = progs.find((p) => p.id === programId) ?? progs.find((p) => p.abbr === 'V5RC') ?? progs[0];
      if (!prog) return;
      setProgram(prog.id!);
  ...
  const doSearch = useCallback(
    (q: string, t: SearchType, prog: number, seas: number | null, reg: string, from: Date, to: Date) => {
      if (q.length < 3 || !seas) {
        setTeams(null);
        setEvents(null);
        setLoading(false);
      ...
      (async () => {
        ...
        try {
          ...
          } else if (eventRes.status === 'rejected' && (teamRes.status === 'rejected' || (teamRes.value ?? []).length === 0)) {
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
          ...
          <View style={styles.section}>
            ...
            {totalPages > 1 && (
              <View style={styles.paginationRow}>
                ...
                >
                  <ChevronLeft size={18} color={eventsPage === 1 ? colors.mutedForeground : colors.foreground} />
                </TouchableOpacity>
                <Text style={styles.pageText}>{eventsPage} / {totalPages}</Text>
                <TouchableOpacity
                ...
                >
                  <ChevronRight size={18} color={eventsPage === totalPages ? colors.mutedForeground : colors.foreground} />
                </TouchableOpacity>
              </View>
            )}
          </View>
      ...
      <Modal visible={filterOpen} transparent animationType="slide" onRequestClose={() => setFilterOpen(false)}>
        ...
        <View style={styles.sheet}>
          ...
          <TouchableOpacity
            style={[styles.typeBtn, styles.typeBtnActive, { alignSelf: 'flex-start', marginTop: 8 }]}

...
const pendingGetAll = new Map<string, Promise<Map<string, VDAStats>>>();
...
const getAll = async (
  ...
  const inflight = pendingGetAll.get(memKey);
  ...
  const promise = (async () => {
    ...
    return map;
  })().finally(() => pendingGetAll.delete(memKey));

  pendingGetAll.set(memKey, promise);
  return promise;
};

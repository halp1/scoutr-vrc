...
export const MyTeamTab = ({
  ...
  const { team: storageTeamNumber } = useStorage();
  const teamNumber = overrideTeamNumber ?? storageTeamNumber;
  ...
  if (!myRanking) {
    return (
      <View style={styles.emptyWrap}>
        ...
        <Text style={styles.emptyBody}>Rankings/schedules have not been posted yet.</Text>
      </View>
    );
  }
                ...
                ? {
                    red: Math.round(
                      row.red.reduce(
                        (a, t) =>
                          a +
                          (vdaMap.get(t.toUpperCase())?.opr ?? vdaMap.get(t)?.opr ?? 0),
                        0,
                      ) *
                        0.5 +
                        row.blue.reduce(
                          (a, t) =>
                            a +
                            (vdaMap.get(t.toUpperCase())?.dpr ?? vdaMap.get(t)?.dpr ?? 0),
                          0,
                        ) *
                          0.5,
                    ),

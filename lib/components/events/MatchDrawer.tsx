...
export const MatchDrawer = ({
  ...
  const showPrediction = predictionsEnabled && vdaMap != null && !row.played;
  const lookup = (teamNum: string) =>
    vdaMap?.get(teamNum.toUpperCase()) ?? vdaMap?.get(teamNum) ?? null;
  const sumOpr = (teams: string[]) =>
    teams.reduce((acc, t) => acc + (lookup(t)?.opr ?? 0), 0);
  const sumDpr = (teams: string[]) =>
    teams.reduce((acc, t) => acc + (lookup(t)?.dpr ?? 0), 0);
  const predictedRed = showPrediction
    ? Math.round(sumOpr(row.red) * 0.5 + sumDpr(row.blue) * 0.5)
    : null;
  const predictedBlue = showPrediction
    ? Math.round(sumOpr(row.blue) * 0.5 + sumDpr(row.red) * 0.5)
    : null;

  const displayRedScore = predictedRed ?? redScore;
  const displayBlueScore = predictedBlue ?? blueScore;
            ...
            <View style={styles.allianceSection}>
              <View style={styles.allianceHeader}>
                ...
                <View style={styles.scoreWithBadge}>
                  ...
                  <Text style={styles.bigScore}>{displayRedScore}</Text>
                </View>
              </View>
              {row.red.map((team, i) => (
            ...
            <View style={styles.allianceSection}>
              <View style={styles.allianceHeader}>
                ...
                <View style={styles.scoreWithBadge}>
                  ...
                  <Text style={styles.bigScore}>{displayBlueScore}</Text>
                </View>
              </View>
              {row.blue.map((team, i) => (
...
const styles = StyleSheet.create({
  ...
  scoreWithBadge: { flexDirection: "row", alignItems: "center", gap: 6 },
  bigScore: {
    fontSize: font["5xl"],
    fontWeight: "500",
    color: colors.foreground,

...
export const CompetitionMode = ({ event, teamId, teamNumber }: Props) => {
  ...
  const { lastRefreshed } = useLivePolling(() => setLiveKey((k) => k + 1));
        ...
        if (!cancelled) {
          ...
          if (myIdx === -1) {
            ...
            const opponentAlliance = match.alliances.find(
              (a) => a.color !== myAlliance?.color,
            );
            const alliance = (myAlliance?.color?.toLowerCase() ?? "red") as
              | "red"
              | "blue";
  ...
  }, [event.id, divId, teamId, teamNumber, liveKey]);

  const allianceColor = nextMatch?.alliance === "red" ? colors.red : colors.blue;
  const allianceBg =
    nextMatch?.alliance === "red" ? colors.red + "20" : colors.blue + "20";

  const refreshTimeLabel = lastRefreshed
    ...
    <View>
      ...
      {refreshTimeLabel && (
        <View style={styles.livePill}>
          ...
          <Text style={styles.liveText}>Live · {refreshTimeLabel}</Text>
        </View>
      )}
      ...
      {loadingMatch ? (
        ...
        >
          <View style={styles.matchTopRow}>
            <Text style={[styles.matchName, { color: allianceColor }]}>
              {nextMatch.name}
            </Text>
              ...
              {nextMatch.matchesUntil > 0 ? (
                <View style={styles.badge}>
                  <Text style={styles.badgeText}>
                    {nextMatch.matchesUntil} match
                    {nextMatch.matchesUntil !== 1 ? "es" : ""} away
                  </Text>
                </View>
              ) : (
                <View

...
export const FieldControllerTab = () => {
	...
	const { connectedDevices, connecting, connectController, disconnectAll, sendMatchMode } =
		useVexSerial();
	...
	const getBtn2Label = () => {
		if (timer.status === TimerStatus.INIT || timer.status === TimerStatus.TIMESUP) return 'Reset';
		return 'Abort';
	};
		...
		<ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
			<View style={styles.profileBar}>
				{profiles.map((p, i) => (
					<Pressable
						key={p.name}
						style={[styles.profilePill, selectedProfile === i && styles.profilePillActive]}
			...
			<View style={styles.timerCard}>
				<Text style={[styles.statusLabel, { color: getModeColor() }]}>{getStatusLabel()}</Text>
						...
						{profile.phases
							...
							.map((ph, i) => (
								<View
									...
									style={[
										styles.phaseDot,
										ph.mode === 'autonomous' ? styles.phaseDotAuto : styles.phaseDotDriver
									]}
								/>
							))}
					</View>
			...
			{profileValid && (
				<View style={styles.buttonsRow}>
					...
					<Pressable style={styles.btn2} onPress={btn2Press}>
						...
						<Text style={[styles.btnLabel, { color: colors.foreground }]}>{getBtn2Label()}</Text>
					</Pressable>
				</View>
			)}

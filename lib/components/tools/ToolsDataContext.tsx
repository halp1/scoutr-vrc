...
interface ToolsDataContextValue {
  ...
  pendingManualEntry: ManualEntry | null;
  navigateToManualEntry: (entry: ManualEntry) => void;
  clearPendingManualEntry: () => void;
}
...
export const ToolsDataProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [pendingManualEntry, setPendingManualEntry] = useState<ManualEntry | null>(null);
  const navigateToManualEntry = useCallback(
    (entry: ManualEntry) => setPendingManualEntry(entry),
    [],
  );
  const clearPendingManualEntry = useCallback(() => setPendingManualEntry(null), []);

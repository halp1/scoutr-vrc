...
export const spansText = (spans: InlineSegment[]): string =>
  spans.map((s) => s.s).join("");
...
export const MANUAL_URL = `${CONSTANTS.PROXY_URL}/manual`;

const fetchManualHTML = async (): Promise<string> => {
  const response = await fetch(MANUAL_URL);
  if (!response.ok) {
    throw new Error(`Failed to load game manual (${response.status}).`);
  }
...
const parseListItemClass = (
  cls: string,
): { itemCls: ListItemClass; violations: boolean } => {
...
const parseBlock = (el: HTMLElement): Block | Block[] | null => {
  ...
  if (tag === "p") {
    if (cls.includes("VRC_Section-Sub-head") || cls.includes("VRC_Section-Header"))
      return null;
      ...
      if (tag === "p") {
        ...
        if (termSpan) {
          ...
          for (const child of node.childNodes) {
            ...
            if (["ul", "ol", "div", "table"].includes(ct))
              addBlock(current!, parseBlock(child));
          }
          continue;
        }
      }

...
export const GameManualTab = () => {
  ...
  useEffect(() => {
    if (!pendingManualEntry || !manual) return;
    ...
    outer: for (const section of manual.sections) {
      for (const sub of section.subsections) {
        if (sub.entries.includes(pendingManualEntry)) {
          found = pendingManualEntry;
    ...
    if (!found) {
      const matchCode = pendingManualEntry.code?.toLowerCase();
      const matchSourceId = pendingManualEntry.sourceId?.toLowerCase();
      const matchTerm = pendingManualEntry.term;
  ...
  const searchResults = searchVisible
    ? searchEntries(searchIndexRef.current, searchQuery)
    : [];
  ...
  const nextSubsection =
    activeSection &&
    currentSubIndex >= 0 &&
    currentSubIndex < activeSection.subsections.length - 1
            ...
            renderItem={({ item }) => (
              <Pressable
                ...
                onPress={() => {
                  ...
                  Keyboard.dismiss();
                  const section =
                    manual.sections.find((s) => s.id === item.sectionId) ?? null;
                  setActiveSection(section);
                  sectionScrollRef.current?.scrollTo({ y: 0, animated: false });
                  openRule(item.entry);
                }}
...
const EntryView = ({
  ...
  return (
    <Animated.View
      ref={entryRef}
      style={[styles.entryContainer, { backgroundColor: flashBg }]}
    >
      {entry.code ? (

...
export const useVexSerial = () => {
	...
	const writeModeToDevice = useCallback((device: Device, deviceId: number, modeBytes: number[]) => {
		...
		for (const portInterface of interfaces) {
			try {
				...
				console.error('[VexSerial] writeBytes error device:', deviceId, 'iface:', portInterface, e);
			}
		}
	}, []);
	...
	const connectController = useCallback(async () => {
		...
		try {
			...
			for (const device of vexDevices) {
				const alreadyListed = connectedDevices.some((d) => d.deviceId === device.deviceId);
				if (alreadyListed) {
					continue;
				}
				try {
					...
					if (isConnected) {
						setConnectedDevices((prev) => {
							...
							return [...prev, { deviceId: device.deviceId, deviceName: `VEX #${device.deviceId}` }];
						});
				...
				const interfaceCount = typeof device.interfaceCount === 'number' ? device.interfaceCount : 1;

...
export const BASE_PATH = "https://www.robotevents.com/api/v2".replace(/\/+$/, "");
...
const drainRateLimitQueue = () => {
  _rateLimitTimer = null;
  if (_rateLimitQueue.length === 0) return;

  const { maxRequests, windowMs } = CONSTANTS.ROBOTEVENTS_RATE_LIMIT;
  const now = Date.now();
  ...
  while (_rateLimitTimestamps.length < maxRequests && _rateLimitQueue.length > 0) {
  ...
  if (_rateLimitQueue.length > 0) {
    const oldest = _rateLimitTimestamps[0];
    const delay = oldest + windowMs - Date.now() + 1;
    _rateLimitTimer = setTimeout(drainRateLimitQueue, Math.max(delay, 0));
  }
};

const acquireRateLimit = (): Promise<void> =>
  new Promise((resolve) => {
    const { maxRequests, windowMs } = CONSTANTS.ROBOTEVENTS_RATE_LIMIT;
    const now = Date.now();
    const cutoff = now - windowMs;
    ...
    if (_rateLimitTimestamps.length < maxRequests) {
      ...
      if (_rateLimitTimer === null) {
        const oldest = _rateLimitTimestamps[0];
        const delay = oldest + windowMs - now + 1;
        _rateLimitTimer = setTimeout(drainRateLimitQueue, Math.max(delay, 0));
      }
    }
  });
...
export class BaseAPI {
  ...
  private async createFetchParams(
    ...
    const overriddenInit: RequestInit & { url?: string } = {
      ...initParams,
        ...
        if (middleware.onError) {
          response =
            (await middleware.onError({
              ...
              error: e,
              response: response ? response.clone() : (undefined as any),
            })) || response;
        }
      }
...
export type InitOverrideFunction = (requestContext: {
  ...
  context: RequestOpts;
}) => PromiseLike<RequestInit & { url?: string }> | (RequestInit & { url?: string });
